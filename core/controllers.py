	...
	def convert_pcap_to_hc22000(self, input, output):
		if not shutil.which('hcxpcapngtool'):
			if self.on_save_error:
				self.on_save_error('hcxpcapngtool not found')
				return

		result = subprocess.run(['hcxpcapngtool', '-o', output, input], text=True, capture_output=True, check=False)
...
class DBController(Callback):
	...
	# Лучше дергать db из основного потока, но так сойдет
	def async_task(func):
